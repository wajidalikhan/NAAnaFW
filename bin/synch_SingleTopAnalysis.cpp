...
int main(int argc, char **argv) {
  ...
  //  cout << "got weights" <<endl;
	...
	fileout_step4<<std::fixed<<std::setprecision(0)<<evtNumber<<std::endl;
        ...
        if(evtNumber==67388229){
	cout <<" Evt Number "<<evtNumber
        <<" | Jet[0] (pt,eta,E) ("<<jets[0].Pt()<<","<<jets[0].Eta()<<","<<jets[0].E()<<")" 
	<<" | Jet[1] (pt,eta,E) ("<<jets[1].Pt()<<","<<jets[1].Eta()<<","<<jets[1].E()<<")" 
	<<" | El[0] (pt,eta,E) "<<tightLep[0].Pt()<<","<<tightLep[0].Eta()<<","<<tightLep[0].E() 
        //<<" | RelIso. "<<selectedIso[0]
	//<<" | bjet Pt "<<bjets[0].Pt()
	<<" | MET "<<met<< " | metPhi "<<metPhi[0] 
	<<" | mt "<< sqrt(2* tightLep[0].Pt() * met* ( 1- cos(fabs(deltaPhi(tightLep[0].Phi(), metPhi[0]))))) 
	<<" | dR[0,1] "<< deltaR(jets[0].Eta(),jets[0].Phi(),tightLep[0].Eta(),tightLep[0].Phi())
	<<" | dR[1,1] "<< deltaR(jets[1].Eta(),jets[1].Phi(),tightLep[0].Eta(),tightLep[0].Phi())
        <<endl;
        }
      }
    }
