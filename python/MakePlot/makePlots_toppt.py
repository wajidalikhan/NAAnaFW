    ...
    for s in samples.itervalues():
        ...
        if(hasattr(s, "components")):
            ...
            for c in s.components:
                ...
                if(opt.sys=="noSys"):
                    filename = opt.resdir+'/res/'+c.label+"_" + opt.channel +".root"
                        ...
                        ( opt.sys=="jesUp" or opt.sys=="jesDown" or opt.sys=="jerUp" or opt.sys=="jerDown" or opt.sys=="topPtWeightUp" ) and
                        ( c.label.startswith("SingleMu") or c.label.startswith("SingleEl") or c.label.startswith("MET") or not(c.label.startswith("TT_")))
                    ):
                    filename = opt.resdir+'/res/'+c.label+"_" + opt.channel +".root"
                else:
            ...
            if opt.sys=="noSys":
                ...
                ( opt.sys=="jesUp" or opt.sys=="jesDown" or opt.sys=="jerUp" or opt.sys=="jerDown" or opt.sys=="topPtWeightUp" ) and
                ( s.label.startswith("SingleMu") or s.label.startswith("SingleEl") or s.label.startswith("MET") or not(s.label.startswith("TT_")))
                ):
                filename = opt.resdir+'/res/'+s.label+"_" + opt.channel +".root"
            else:
...
#            infile_nEvt = ROOT.TFile.Open(filename)
    ...
    maximum = max([stack_bkg.GetMaximum(),stack_data.GetMaximum(), stack_sig.GetMaximum()])
    minimum = min([h_bkg.GetMinimum(),h_data.GetMinimum(), stack_sig.GetMinimum()])

    print "bkg ", stack_bkg.GetMaximum(), " data ", stack_data.GetMaximum(), " sig ", stack_sig.GetMaximum()
